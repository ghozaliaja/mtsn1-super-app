        ...
        try {
            ...
            if (exportScope === 'class') {
                ...
                if (exportPeriod === 'daily') {
                    ...
                    // Add Header Info
                    titleInfo = [
                        [`Rekap ${viewMode === 'school' ? 'Kehadiran' : 'Ibadah'} Bulanan Siswa MTsN 1 Labuhanbatu`],
                        [`Kelas : ${selectedClass}`],
            ...
            // Explicit Header Ordering to fix "Nama di belakang" issue
            let customHeader: string[] | undefined = undefined;
            if (exportPeriod === 'monthly' && exportScope === 'class' && viewMode === 'school') {
                const daysInMonth = new Date(parseInt(selectedMonth.split('-')[0]), parseInt(selectedMonth.split('-')[1]), 0).getDate();
                customHeader = ['No', 'Nama', 'Kelas'];
                for (let i = 1; i <= daysInMonth; i++) customHeader.push(String(i));
                customHeader.push('Total Hadir', 'Total Terlambat');
            }

            const ws = XLSX.utils.json_to_sheet(data, { origin, header: customHeader } as any);
